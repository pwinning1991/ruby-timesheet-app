...
RSpec.describe User, type: :model do
  ...
  describe "validations" do
    ...
    it 'requires the phone attr to only integers' do
      @user.phone = 'mygreatstr'
      expect(@user).to_not be_valid
    end
    it 'requires the ssn  attr to only integers' do
      @user.ssn = 'mygreatstr'
      expect(@user).to_not be_valid
    end

    it 'requires the phone attr to only have 10 characters' do
      @user.phone = '12344312345'
      expect(@user).to_not be_valid
    end
  ...
  describe "realtionship between admins and employees" do
